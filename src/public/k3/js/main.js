$(document).ready(function() {
    $.ajax({
        type: "POST",
        url: "/api/webapi/k3/GetPercent",
        dataType: "json",
        success: function (response) {
            if (response.status) {
                $('#txtBig').text(response.percent);
                $('#txtSmall').text(response.percent);
            }
        }
    });
});
...
document.addEventListener('DOMContentLoaded', function() {
    // load mặc định phòng chơi 5 phút
    $('html').attr('data-dpr', 5);
});
...
function totalMoney() {
    ...
    $('.result').text(formatMoney(result) + ' ' + '$');
}
