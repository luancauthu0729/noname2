...
async function plusMoney(game) {
    ...
    `SELECT id, phone, bet, price, money, fee, amount, result, typeGame 
     FROM result_k3 
     WHERE status = 0 AND game = ? FOR UPDATE`,
    [game]
);
    const [k3Setting] = await connection.execute(`SELECT percent FROM k3_admin`);
    let percent = k3Setting[0].percent;
...
const GetPercent = async (req, res) => {
    ...
    const [percents] = await connection.query('SELECT percent FROM k3_admin');
    let percent = percents[0].percent;
