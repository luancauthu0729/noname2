...
const updateMoney = async (req, res) => {
    ...
    if(type == 0){
        let [users] = await connection.query('SELECT `money` FROM users WHERE `phone` = ? ', [phone]);
        const user = users?.[0];
        if(user.money < money){
             return res.status(200).json({
                message: 'fail',
...
const deleted = async (req, res) => {
    ...
    if (!auth || !id) {
      ...
      await connection.query(`DELETE FROM point_list WHERE phone = ?`, [phone]);
        await connection.query(`DELETE FROM turn_over WHERE phone = ?`, [phone]);
    return res.status(200).json({
...
const settingK3 = async (req, res) => {
    try {
        ...
        await connection.query(`UPDATE k3_admin SET percent = ?`, [percent]);
        return res.status(200).json({
