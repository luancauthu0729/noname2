...
const addBank = async (req, res) => {
   ...
   // const [user_bank] = await connection.query('SELECT * FROM user_bank WHERE tinh = ? ', [tinh]);
    ...
    if (user_bank2.length == 0) {
        ...
        await connection.query('UPDATE user_bank SET name_bank = ?, name_user = ?, stk = ?, email = ?, sdt = ?, tinh = ?, time = ? WHERE phone = ?', [name_bank, name_user, stk, '', '', '', time, userInfo.phone]);
        return res.status(200).json({
    ...
    if (user_bank.length != 0) {
        if (withdraw.length < 3) {
            if (userInfo.money - money >= 0) {
                ...
                return res.status(200).json({
                    message: 'Số dư không đủ để thực hiện',
                    status: false,
                    timeStamp: timeNow,
                });
            }
            ...
            return res.status(200).json({
                message: 'Chỉ được rút tối đa 2 lần/ngày',
                status: false,
                timeStamp: timeNow,
            });
        }
        ...
        return res.status(200).json({
            message: 'Vui lòng thêm tài khoản ngân hàng trước khi rút tiền',
            status: false,
            timeStamp: timeNow,
        });
    }
